...
export async function GET(request) {
  try {
    ...
    if (parentId !== null && parentId !== undefined) {
      query.parent = parentId === 'null' || parentId === '' ? null : parentId;
    }
...
export async function POST(request) {
  try {
    ...
    const processedImages = (images || []).filter(img => img.url && img.url.trim() !== '');
    ...
    const category = await Category.create({
      ...
      parent: (parent && parent !== '' && parent !== 'null') ? parent : null,
    ...
    if (categoriesWithChildren.length > 0) {
      return NextResponse.json(
        { 
          ...
          affectedCategories: categoriesWithChildren.length
        },
        { status: 400 }
      );
    }

    // Optional: Check if categories have products
    // const productsCount = await Product.countDocuments({ category: { $in: ids } });
    // if (productsCount > 0) {
    //   return NextResponse.json(
    //     { 
    //       success: false,
    //       error: `Cannot delete categories with ${productsCount} associated products`
    //     },
    //     { status: 400 }
    //   );
    // }
